-- Fix ambiguous column reference in evaluate_tenders_batch function
CREATE OR REPLACE FUNCTION public.evaluate_tenders_batch(_org_id uuid, _profile_ids uuid[])
 RETURNS TABLE(evaluation_id uuid, tender_id uuid, profile_id uuid, total_score integer, all_minimum_met boolean, matched_keywords jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
