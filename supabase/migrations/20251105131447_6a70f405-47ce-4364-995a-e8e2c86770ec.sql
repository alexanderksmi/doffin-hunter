-- Fix ARRAY_AGG with DISTINCT and ORDER BY in evaluate_tenders_batch
...
BEGIN
  ...
  profile_criteria AS (
    SELECT 
      ...
      -- Compute stable MD5 hash of criteria (sort arrays after aggregation)
      md5(
        concat(
          coalesce((
            SELECT string_agg(kw, ',' ORDER BY kw)
            FROM unnest(ARRAY_AGG(DISTINCT mr.keyword) FILTER (WHERE mr.keyword IS NOT NULL)) as kw
          ), ''),
          '|',
          coalesce((
            SELECT string_agg(kw || ':' || (JSONB_OBJECT_AGG(sk.keyword, sk.weight) FILTER (WHERE sk.keyword IS NOT NULL)->>kw)::text, ',' ORDER BY kw)
            FROM unnest(ARRAY_AGG(DISTINCT sk.keyword) FILTER (WHERE sk.keyword IS NOT NULL)) as kw
          ), ''),
          '|',
          coalesce((
            SELECT string_agg(kw, ',' ORDER BY kw)
            FROM unnest(ARRAY_AGG(DISTINCT nk.keyword) FILTER (WHERE nk.keyword IS NOT NULL)) as kw
          ), ''),
          '|',
          coalesce((
            SELECT string_agg(code || ':' || w::text, ',' ORDER BY code)
            FROM (
              SELECT cc.cpv_code as code, cc.weight as w
              FROM cpv_codes cc
              WHERE cc.profile_id = cp.id
            ) AS cpv_data
          ), '')
        )
      ) as fingerprint
