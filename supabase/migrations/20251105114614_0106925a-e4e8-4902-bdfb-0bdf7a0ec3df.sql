-- Migration 2: Set-Based SQL Evaluation and CPV Hierarchy (fixed)
...
CREATE INDEX IF NOT EXISTS idx_cpv_hierarchy_parent ON public.cpv_hierarchy(parent_code);
CREATE INDEX IF NOT EXISTS idx_cpv_hierarchy_level ON public.cpv_hierarchy(level);
...
BEGIN
      ...
      (
        SELECT COALESCE(SUM(
          GREATEST(cpv_match_weight(tc, pc_cpv) * 
            COALESCE((SELECT weight FROM cpv_codes WHERE cpv_code = pc_cpv AND profile_id = pc.profile_id LIMIT 1), 1), 0)
        ), 0)
  ...
  SELECT
    ...
    e.t_id,
    e.p_id,
    CASE WHEN NOT e.all_min_met THEN 0 WHEN NOT e.no_negatives THEN 0 ELSE e.supp_score + e.cpv_sc END as total_sc,
    e.all_min_met,
    JSONB_BUILD_OBJECT(
...
-- Helper function: Upsert evaluation results
CREATE OR REPLACE FUNCTION public.upsert_evaluation_results(_org_id uuid, _lead_profile_id uuid, _partner_profile_id uuid, _combination_type text, _results JSONB)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  INSERT INTO tender_evaluations (tender_id, organization_id, lead_profile_id, partner_profile_id, combination_type, all_minimum_requirements_met, support_score, negative_score, cpv_score, total_score, matched_support_keywords, matched_negative_keywords, matched_cpv_codes, explanation)
  SELECT (r->>'tender_id')::uuid, _org_id, _lead_profile_id, _partner_profile_id, _combination_type, (r->'matched_keywords'->>'all_minimum_met')::boolean, (r->'matched_keywords'->>'support_score')::integer, 0, (r->'matched_keywords'->>'cpv_score')::integer, (r->>'total_score')::integer, COALESCE(r->'matched_keywords'->'support', '[]'::jsonb), COALESCE(r->'matched_keywords'->'negative', '[]'::jsonb), COALESCE(r->'matched_keywords'->'cpv', '[]'::jsonb), r->'matched_keywords'->>'explanation'
  ...
  DO UPDATE SET all_minimum_requirements_met = EXCLUDED.all_minimum_requirements_met, support_score = EXCLUDED.support_score, negative_score = EXCLUDED.negative_score, cpv_score = EXCLUDED.cpv_score, total_score = EXCLUDED.total_score, matched_support_keywords = EXCLUDED.matched_support_keywords, matched_negative_keywords = EXCLUDED.matched_negative_keywords, matched_cpv_codes = EXCLUDED.matched_cpv_codes, explanation = EXCLUDED.explanation, updated_at = now();
END;
$$;

-- Function to claim next evaluation job
CREATE OR REPLACE FUNCTION public.claim_next_evaluation_job()
RETURNS TABLE(job_id uuid, organization_id uuid, affected_profile_ids uuid[])
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  SELECT id INTO v_job_id FROM evaluation_jobs WHERE status = 'pending' AND run_not_before <= now() ORDER BY run_not_before ASC, created_at ASC LIMIT 1 FOR UPDATE SKIP LOCKED;
  IF v_job_id IS NULL THEN RETURN; END IF;
  UPDATE evaluation_jobs SET status = 'running', started_at = now(), updated_at = now() WHERE id = v_job_id;
  RETURN QUERY SELECT ej.id, ej.organization_id, ej.affected_profile_ids FROM evaluation_jobs ej WHERE ej.id = v_job_id;
END;
$$;