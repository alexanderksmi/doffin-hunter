-- Simplify criteria fingerprint calculation
...
BEGIN
  ...
  criteria_fingerprints AS (
    SELECT 
      ...
      md5(
        concat(
          coalesce((SELECT string_agg(keyword, ',' ORDER BY keyword) FROM minimum_requirements WHERE profile_id = cp.id), ''),
          '|',
          coalesce((SELECT string_agg(keyword || ':' || weight, ',' ORDER BY keyword) FROM support_keywords WHERE profile_id = cp.id), ''),
          '|',
          coalesce((SELECT string_agg(keyword, ',' ORDER BY keyword) FROM negative_keywords WHERE profile_id = cp.id), ''),
          '|',
          coalesce((SELECT string_agg(cpv_code || ':' || weight, ',' ORDER BY cpv_code) FROM cpv_codes WHERE profile_id = cp.id), '')
        )
      ) as fingerprint
