-- Drop and recreate evaluate_tenders_batch with correct return signature
...
CREATE FUNCTION public.evaluate_tenders_batch(_org_id uuid, _profile_ids uuid[])
 RETURNS TABLE(tender_id uuid, profile_id uuid, total_score integer, all_minimum_met boolean, matched_keywords jsonb)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT
    e.t_id,
    e.p_id,
    CASE WHEN NOT e.all_min_met THEN 0 WHEN NOT e.no_negatives THEN 0 ELSE e.supp_score + e.cpv_sc END,
    e.all_min_met,
    JSONB_BUILD_OBJECT(
