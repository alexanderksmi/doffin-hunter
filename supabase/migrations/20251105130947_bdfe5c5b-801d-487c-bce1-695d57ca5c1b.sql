-- Drop old upsert function and create new one with cleanup
DROP FUNCTION IF EXISTS public.upsert_evaluation_results(uuid, uuid, uuid, text, jsonb);

CREATE FUNCTION public.upsert_evaluation_results_with_cleanup(
  _org_id uuid, 
  _profile_id uuid,
  _combination_type text,
  _results jsonb,
  _criteria_fingerprint text
)
