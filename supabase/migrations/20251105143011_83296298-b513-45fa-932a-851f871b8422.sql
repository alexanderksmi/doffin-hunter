-- Update evaluate_tenders_batch to include matched minimum requirements
...
BEGIN
  ...
  evaluated AS (
    SELECT
      ...
      (
        SELECT JSONB_AGG(JSONB_BUILD_OBJECT('keyword', req, 'type', 'minimum'))
        FROM unnest(pc.minimum_reqs) as req
        WHERE ft.search_vector @@ plainto_tsquery('norwegian', req)
      ) as matched_minimum,
      (
  ...
  SELECT
    ...
    e.all_min_met,
    JSONB_BUILD_OBJECT(
      'minimum', COALESCE(e.matched_minimum, '[]'::jsonb),
